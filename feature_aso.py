...
class feature_generator:
    ...
    def __init__(self, time, data=None):
        ...
        self._data = data
        if data is None:
            self._data = load_weather_library(target_list)
        if time == 16:
...
def main():
    ...
    #exit()
    weather_library = load_weather_library(target_list)
    weather_dataset = weather_library[target_list[0]]
    print("--wind--")
    for i in range(24):
        hoge = get_measurement_value(dt(2016, 1, 1,  i), weather_dataset, "風向")
        print(hoge)
    print("--temperature--")
    for i in range(24):
        hoge = get_measurement_value(dt(2015, 12, 18,  i), weather_dataset, "気温")
        print(hoge)
