...
for i in range(1,days):
	...
	print(_date)
	html_bytes = aso_node.get_data(_type="hourly", date=_date)
	html_lines = resolve(html_bytes)
	data = amp.get_data(html_lines, _date)
	#print(data)
	aso_lines += [",".join(x) for x in data]
# 最新のデータを入手
html_bytes = aso_node.get_data(_type="real-time")
html_lines = resolve(html_bytes)
...
for i in range(1,days):
	_date = dt.now()-td(days=i)
	print(_date)
	html_bytes = otohime_node.get_data(_type="hourly", date=_date)
	html_lines = resolve(html_bytes)
	data = amp.get_data(html_lines, _date)
	#print(data)
	otohime_lines += [",".join(x) for x in data]
# 最新のデータを入手
